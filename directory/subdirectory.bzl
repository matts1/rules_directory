...
def _subdirectory_impl(ctx):
    dir = get_relative(ctx.attr.parent[DirectoryInfo], ctx.attr.path, require_dir = True)
    return [
        dir,
...
subdirectory = rule(
    ...
    attrs = {
        "parent": attr.label(
            ...
            mandatory = True,
            doc = "A label corresponding to the parent directory.",
        ),
        "path": attr.string(
            mandatory = True,
            doc = "A relative path to the parent directory",
        ),
    },
